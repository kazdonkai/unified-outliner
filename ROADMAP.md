...
- **Editable tables**: Extend the fenced-code-block Move/Delete/Partial-Edit support to Markdown tables, designing a safe write-back rule that preserves table syntax (delimiter row, column alignment) on Apply; cell-level structured editing and a full Table Mode are tracked separately, beyond this next step.
