...
- **tableのraw編集と安全な書き戻し（Phase 5E-2A、次の対象）**: fenced code blockで実現した移動・削除・Partial Edit Paneでの生Markdown編集と同等の対応をMarkdown tableにも広げる。table構文（区切り行・セル整列など）を壊さない安全な書き戻し規則の設計が主な検討事項であり、cell単位の構造化編集やTable Modeの本格実装は本フェーズの対象外として別途検討する。
