...
- **Show mirrors in the tree**: turn on **Show mirror embeds in Outline Tree** (off by default). Each embed line then appears as a read-only **⧉ Mirror: …** row. Clicking the row (or pressing Enter) moves the cursor to the embed line itself, like any other row; to go to the referenced heading or block, double-click the row, choose **Go to mirror source** from its context menu, or — on mobile — tap the already-selected row again. A mirror whose target cannot be found is marked "(not found)" and one that forms a circular reference "(circular)", both in the theme's error color. With the setting off, embed lines appear as ordinary paragraphs.
