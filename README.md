...
- A standalone callout or blockquote, and an extended block (**List + Callout**/**List + Quote**), can be moved and opened in the Partial Edit Pane directly from the Outline Tree View (see Visual guide and "Edit a focused subtree" above); an extended block's two members are edited together and saved in one Apply whenever their structure allows a clean split. A standalone fenced code block (including Mermaid) can be optionally shown in the Outline Tree as a read-only row, and from there moved up/down (same parent only), deleted as one unit, and opened in the Partial Edit Pane (the fence lines are hidden from the editable text, with a separate language selector above it); a Markdown table can likewise be optionally shown as a read-only row and opened in the Partial Edit Pane as raw Markdown (Apply rejects an edit that breaks the table's structure, leaving the note unchanged), but still has no move or delete support from the tree. **Move block** can still move either kind as a whole when the cursor is inside it in the body editor, regardless of the Outline Tree setting.
...
Structural move and level commands, Outline Tree navigation and editing, a Partial Edit Pane with structured marker-free editing for list items and their direct children (add, delete, reorder, indent/outdent, inline edit, one level at a time), and structured editing for standalone callouts/blockquotes and extended blocks are now available (see above). A standalone fenced code block can optionally be shown in the Outline Tree and, from there, moved, deleted, and opened in the Partial Edit Pane, where its fence lines are hidden from the editable text and its language is chosen from a separate selector above it; a Markdown table can likewise optionally be shown in the Outline Tree and opened in the Partial Edit Pane as raw Markdown, with Apply checking the result is a structurally valid table, but table move, delete, and cell-level editing are not supported yet. Table move/delete and a lightweight Table Mode are next up; free movement to an arbitrary depth or parent, full subtree-level operations beyond one level of indent/outdent, drag-and-drop inside the Partial Edit Pane, and richer code-block editing (cell/body-aware editing, not just raw fence-to-fence text) all remain future work, not yet started.
