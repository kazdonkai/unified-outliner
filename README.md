...
The **Partial Edit Pane** opens the selected section, list subtree, paragraph, standalone callout/blockquote, fenced code block, table, or extended block in a dedicated editor. Make your changes, then select **Apply** to write them back to the source note — the note's Markdown is always the single source of truth; the pane's structured controls (described below) are only a view onto it, never a separate model that could drift from what the note actually contains. If the source area changed after the pane opened, the pane protects the note by refusing to apply conflicting content; reload the target and review the change instead of overwriting it. If the note changes elsewhere — another pane, the Outline Tree, or an Undo/Redo in the body editor — while this pane is open with no unsaved changes, it resynchronizes automatically to match, including switching its own display between a leaf item's editor and a parent item's editor when that change adds or removes the last child.

For a paragraph, callout, blockquote, fenced code block, or table that has a block id (`^id` — for example one added by **Create mirror**), the id is kept out of the editor and shown in its own **Block ID:** field below it. Edit the field to rename the id, or clear it to remove the id; Apply writes it back in the same place it was (at the end of the last line, or on its own line after the block). The field is shown only for a block that has an id, and a value other than letters, digits, and hyphens is refused. When you rename an id, the mirrors in the same note that embed it (`![[#^id]]`) are updated in the same edit (one Undo step). Removing an id leaves those mirrors without a target, and you are warned. If links or embeds in other notes point at the id, a warning tells you they may now be broken; other notes are never changed.
...
- The Partial Edit Pane's **Block ID** field changes only the block's own id and, when the id is renamed, the same-note mirror embeds that use it — in one edit. Removing an id, or an id that other notes link to, is warned about; other notes are never modified. Only letters, digits, and hyphens are accepted.
...
Version 1.0 completes the plugin's core scope for editing a single note: structural move and level commands; Outline Tree navigation, renaming, drag and drop (including across sections), and insertion; a Partial Edit Pane with structured, marker-free editing for list items and their direct children (add, delete, reorder, indent/outdent, inline edit, one level at a time) and structured editors for callouts, blockquotes, extended blocks, fenced code blocks, and tables (including Table Mode), with a block's `^id` edited in its own Block ID field; block copy (Copy / Duplicate / Paste); and same-note mirrors (show, create, move, delete, and see which mirrors reference a block).

Not yet started, and possible future work: cross-section support for the **Move up/down** commands themselves, free movement to an arbitrary depth or parent, subtree-level operations beyond one level of indent/outdent, drag and drop inside the Partial Edit Pane, richer fenced-code-block editing, for mirrors, editing through a mirror row, drag and drop of mirror rows, and mirrors of other notes; and, for block ids, adding a new id from the Partial Edit Pane and checking a new id for duplicates.
