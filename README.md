...
- **Show tables in Outline Tree View**: shows a standalone Markdown table as its own read-only row in the tree, labeled by its header columns. Clicking a row jumps to its header row; tables have no move, delete, or Partial Edit support from the tree. Off by default.
...
Structural move and level commands, Outline Tree navigation and editing, a Partial Edit Pane with structured marker-free editing for list items and their direct children (add, delete, reorder, indent/outdent, inline edit, one level at a time), and structured editing for standalone callouts/blockquotes and extended blocks are now available (see above). A standalone fenced code block can optionally be shown in the Outline Tree and, from there, moved, deleted, and opened in the Partial Edit Pane as raw Markdown; a Markdown table can optionally be shown in the Outline Tree but remains read-only there. Editable tables (a Partial Edit Pane / raw editing path for Markdown tables, matching what fenced code blocks now have) are next up; free movement to an arbitrary depth or parent, full subtree-level operations beyond one level of indent/outdent, drag-and-drop inside the Partial Edit Pane, and richer code-block editing (cell/body-aware editing, not just raw fence-to-fence text) all remain future work, not yet started.
