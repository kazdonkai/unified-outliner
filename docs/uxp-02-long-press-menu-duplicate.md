...
作成日: 2026-08-12
状態: 実装完了、test/tsc/lint/build/diff-check 済み。iPad・デスクトップ実機で多重表示の解消を確認済み（2026-08-13）。ブランチ `fix/uxp-02-long-press-menu-duplicate`（`main` 未マージ）。
...
2026-08-13、`ipad-test` vault（Obsidian Sync で Mac と iPad を同期）に本修正のビルドを配置しユーザーが実機で確認。§1の再現手順（ある行を長押しでメニュー表示 → 閉じずに別の行を長押し）を試行し、**前のメニューが自動的に閉じ、多重に積み重なる状態は再発しないことを確認した**（ユーザー報告: 「多重化はしなくなった」)。

### デスクトップ

2026-08-13、同じくユーザーが確認。マウス右クリックでの多重化も発生しないことを確認した（ユーザー報告: 「多重化はしなくなった」)。デスクトップは§3で述べたとおり元々OSネイティブメニューのため多重化自体起きない環境だが、本修正適用後もその前提が崩れていないことを含めて確認済み。

### モバイルエミュレーション（任意）

未実施（iPad実機で直接確認できたため省略）。

## 10. 既知の制約・未解決事項

- iPad・デスクトップともに、§1の多重表示バグそのものが解消されたことはユーザー実機確認により確認済み。ただし、UXP-01 の12項目チェックリスト・本ドキュメント§9で言及した項目それぞれについて、今回は「多重化が起きないこと」を中心にピンポイントで確認したものであり、UXP-01のときのような項目別の逐一確認は行っていない。UXP-01側の既存機能（ドラッグハンドル、mouse drag、右クリックメニュー等）に回帰がないかは、本チケットの本流の確認事項ではないため、気になる場合は別途確認を依頼する。
- UXP-01 との因果関係なしの判定根拠は §3 のとおり diff ベースで確認済み。
