...
| モバイルで、選択済みの行をもう一度タップ | 参照先の先頭行（同上） |
...
`tests/phase5m2MirrorRowNavigation.test.ts`（28 件）を追加した。クリック先（見出し・行末 ID・単独 ID 行・未解決・循環）、参照先への移動先と拒否理由、モバイルの再タップ判定（未選択・フォーカスなし・長押し・時間不明・デスクトップ）、ダブルクリック後のクリック抑止、日英の文言、配線の静的チェックを確かめる。既存テストは、Phase 5M-0 のジャンプ先の静的チェック 1 件と、Phase 5M-2 のメニュー構成の静的チェック 1 件を新しい仕様に合わせて更新した。
